...
/**
 * @description  changed random number generator to something more fun
 * @param max
 * @returns {number}
...
Enemy.prototype.update = function (dt) {
   ...
   if (player.x < this.x + 60 && player.x + 37 > this.x && player.y < this.y + 25 && 30 + player.y > this.y) {
      ...
      if (player.y < 0) {
         this.addScore(10000); // linear increase
 ...
 * @param points
 */
Enemy.prototype.clearScore = function () {
   this.score = 0;
};

Enemy.prototype.addScore = function () {
...
/**
 * @description pretty print numbers
 * @param x
 * @returns {string}
...
var Player = function () {
   ...
   _createClass(Player, [{
      ...
      key: 'parseInput',
      value: function parseInput(keyCode) {
         switch (keyCode) {
            case 'up':
...
var Engine = function () {
  var global = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this;
